...
#define DETECT_THRESHOLD_REGEX                                                                     \
    "^,\\s*type\\s*(limit|both|threshold)\\s*,\\s*track\\s*(by_dst|by_src|by_both|by_triple|by_rule)\\s*,"   \
    "\\s*count\\s*(\\d+)\\s*,\\s*seconds\\s*(\\d+)\\s*$"

/* TODO: "apply_to" */
#define DETECT_RATE_REGEX "^,\\s*track\\s*(by_dst|by_src|by_both|by_triple|by_rule)\\s*,\\s*count\\s*(\\d+)\\s*,\\s*seconds\\s*(\\d+)\\s*,\\s*new_action\\s*(alert|drop|pass|log|sdrop|reject)\\s*,\\s*timeout\\s*(\\d+)\\s*$"
        ...
        uint8_t *ret_parsed_new_action, char **ret_th_ip)
{
    ...
    switch (rule_type) {
        ...
        case THRESHOLD_TYPE_RATE:
            ...
            else if (strcasecmp(th_track, "by_triple") == 0) {
                parsed_track = TRACK_TRIPLE;
            }
